...
class _ChartsSectionState extends State<ChartsSection> {
  List<EmprestimosPorDiaMesResponse> _emprestimosPorMes = [];
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final data = await provider.getEmprestimosPorMesResponse();
      setState(() {
        _emprestimosPorMes = data;
        _isLoading = false;
      });
    } catch (e) {
      setState(() {
  ...
  Future<void> _refreshData() async {
    ...
    // Não mostrar loading para atualizações automáticas para evitar saltos no scroll
    final provider = context.read<EmprestimosDiaProvider>();
    final data = await provider.refreshEmprestimosPorMes();
  ...
  Widget build(BuildContext context) {
    ...
    // Recarregar dados quando o provider for atualizado
    if (provider.emprestimosPorDia.length != _lastUpdateCount && !_isLoading) {
      _lastUpdateCount = provider.emprestimosPorDia.length;
      // Usar microtask para evitar problemas de timing e minimizar impacto no scroll
      Future.microtask(() {
    ...
    final List<FlSpot> spots = _emprestimosPorMes
        .map((e) => FlSpot(e.diaMes.toDouble(), e.totalEmprestimos.toDouble()))
        .toList();
    final dias = spots.map((e) => e.x).toList();
    final minX =
        dias.isNotEmpty ? dias.reduce((a, b) => a < b ? a : b).toDouble() : 0.0;
    final maxX =
        dias.isNotEmpty ? dias.reduce((a, b) => a > b ? a : b).toDouble() : 0.0;
