...
class FuncionarioService {
  ...
  Future<List<Funcionario>> getFuncionarios() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return jsonList
            .map<Funcionario>((json) => Funcionario.fromJson(json))
            .toList();
      } else {
        throw Exception(
            'Falha ao carregar funcionários: ${response.statusCode}');
      }
    } on Exception {
      return [];
    }
  ...
  Future<void> deleteFuncionario(String email) async {
    try {
  ...
  Future<List<Funcionario>> getFuncionariosSemCartao() async {
    final response = await http.get(Uri.parse('$_baseUrl/nao_associados'),
        headers: await getHeaders());
    if (response.statusCode == 200) {
      final List<dynamic> jsonList = json.decode(response.body);
      return jsonList
  ...
  Future<void> updateFuncionario(Funcionario funcionario) async {
    try {
      ...
      if (response.statusCode != 200 && response.statusCode != 204) {
        throw Exception(
            'Erro ao atualizar funcionário: ${response.statusCode}');
      }
    } catch (e) {
